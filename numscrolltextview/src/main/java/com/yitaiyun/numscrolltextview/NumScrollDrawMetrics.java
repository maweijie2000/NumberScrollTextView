...
/**
 * This wrapper class represents some core drawing metrics that {@link NumScrollView} and
 * {@link NumScrollColumnManager} require to calculate the positions and offsets for rendering
 * the text onto the canvas.
 *
 * @author Jin Cao
 */
class NumScrollDrawMetrics {
    ...
    NumScrollDrawMetrics(Paint textPaint) {
    ...
    float getCharWidth(char character) {
        if (character == NumScrollUtils.EMPTY_CHAR) {
            return 0;
        }
