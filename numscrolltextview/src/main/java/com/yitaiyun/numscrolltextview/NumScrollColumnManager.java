...
/**
 * In ticker, each character in the rendered text is represented by a {@link NumScrollColumn}. The
 * column can be seen as a column of text in which we can animate from one character to the next
 * by scrolling the column vertically. The {@link NumScrollColumnManager} is then a
 * manager/convenience class for handling a list of {@link NumScrollColumn} which then combines into
 * the entire string we are rendering.
 */
@SuppressWarnings("ForLoopReplaceableByForEach")
class NumScrollColumnManager {
    final ArrayList<NumScrollColumn> tickerColumns = new ArrayList<>();
    private final NumScrollDrawMetrics metrics;

    private NumScrollCharacterList[] characterLists;
    private Set<Character> supportedCharacters;

    NumScrollColumnManager(NumScrollDrawMetrics metrics) {
    ...
    void setCharacterLists(String... characterLists) {
        this.characterLists = new NumScrollCharacterList[characterLists.length];
        for (int i = 0; i < characterLists.length; i++) {
            this.characterLists[i] = new NumScrollCharacterList(characterLists[i]);
        }
    ...
    NumScrollCharacterList[] getCharacterLists() {
    ...
    void setText(char[] text) {
        ...
        for (int i = 0; i < tickerColumns.size(); ) {
            final NumScrollColumn tickerColumn = tickerColumns.get(i);
            ...
            switch (actions[i]) {
                case LevenshteinUtils.ACTION_INSERT:
                    tickerColumns.add(columnIndex,
                            new NumScrollColumn(characterLists, metrics));
                ...
                case LevenshteinUtils.ACTION_DELETE:
                    tickerColumns.get(columnIndex).setTargetChar(NumScrollUtils.EMPTY_CHAR);
                    columnIndex++;
                    break;
                default:
    ...
    void onAnimationEnd() {
        for (int i = 0, size = tickerColumns.size(); i < size; i++) {
            final NumScrollColumn column = tickerColumns.get(i);
