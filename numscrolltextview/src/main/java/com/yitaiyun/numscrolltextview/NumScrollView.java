...
package com.yitaiyun.numscrolltextview;
...
public class NumScrollView extends View {
    ...
    private final NumScrollDrawMetrics metrics = new NumScrollDrawMetrics(textPaint);
    private final NumScrollColumnManager columnManager = new NumScrollColumnManager(metrics);
    ...
    public NumScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0, 0);
    }

    public NumScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr, 0);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public NumScrollView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.NumScrollView,
                defStyleAttr, defStyleRes);

        final int textAppearanceResId = arr.getResourceId(
                R.styleable.NumScrollView_android_textAppearance, -1);

        // Check textAppearance first
        if (textAppearanceResId != -1) {
            final TypedArray textAppearanceArr = context.obtainStyledAttributes(
                    textAppearanceResId, R.styleable.NumScrollView);
        ...
        this.animationDurationInMillis = arr.getInt(
                R.styleable.NumScrollView_numScroll_animationDuration, DEFAULT_ANIMATION_DURATION);
        this.animateMeasurementChange = arr.getBoolean(
                R.styleable.NumScrollView_numScroll_animateMeasurementChange, false);
        ...
        final int defaultCharList =
                arr.getInt(R.styleable.NumScrollView_numScroll_defaultCharacterList, 0);
        switch (defaultCharList) {
            case 1:
                setCharacterLists(NumScrollUtils.provideNumberList());
                break;
            case 2:
                setCharacterLists(NumScrollUtils.provideAlphabeticalList());
                break;
            default:
                if (isInEditMode()) {
                    setCharacterLists(NumScrollUtils.provideNumberList());
    ...
    private class StyledAttributes {
        ...
        void applyTypedArray(TypedArray arr) {
            gravity = arr.getInt(R.styleable.NumScrollView_android_gravity, gravity);
            shadowColor = arr.getColor(R.styleable.NumScrollView_android_shadowColor,
                    shadowColor);
            shadowDx = arr.getFloat(R.styleable.NumScrollView_android_shadowDx, shadowDx);
            shadowDy = arr.getFloat(R.styleable.NumScrollView_android_shadowDy, shadowDy);
            shadowRadius = arr.getFloat(R.styleable.NumScrollView_android_shadowRadius,
                    shadowRadius);
            text = arr.getString(R.styleable.NumScrollView_android_text);
            textColor = arr.getColor(R.styleable.NumScrollView_android_textColor, textColor);
            textSize = arr.getDimension(R.styleable.NumScrollView_android_textSize, textSize);
            textStyle = arr.getInt(R.styleable.NumScrollView_android_textStyle, textStyle);
        }
    }
     ...
     * <p>
     * <p>You can find some helpful character list in {@link NumScrollUtils}.
     *
